...
function api_data_sign(post_data, token){
    ...
    for (var i=0; i<keys.length; i++){  // 遍历数组中的键到post_data中取值
        if(post_data[keys[i]] == null){
            post_data[keys[i]] = '';
        }
        post_data_string += keys[i];

        if(typeof post_data[keys[i]]=='string') {
            post_data_string += post_data[keys[i]].replace(/(^\s*)|(\s*$)/g, "");
        }else{
 ...
 * @param _self,_self
 */
function pop_error_msg(msg,_self) {
...
(function() {
    ...
    hm.src = "https://hm.baidu.com/hm.js?52db8bc10ae231050e5957dd40e3694f";
