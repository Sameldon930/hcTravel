...
var WIS_toast = function(config){
    ...
    htmlEntity.hide();
    htmlEntity.fadeIn(time/3);
    htmlEntity.fadeOut(time/3);
...
function wis_cache_time(time_form,dir_form){
    ...
    if(dir_form == 'set'){
        ...
        window.localStorage.setItem('WIS_G.time',JSON.stringify(torage_time));
        return torage_time[time_form];
    }
    if(dir_form == 'get'){
        var torage_time = window.localStorage.getItem('WIS_G.time');
...
var WIS_ajax = function(){
    //获取用户信息
    var obj=JSON.parse(window.localStorage.getItem('WIS_G.user'));
...
function hideShowPsw  (e) {
    ...
    if($('#'+id_type).attr('type') == 'password'){
        ...
        $('#'+img).attr('src',"i/hide@2x.png");
    }else{
        $('#'+id_type).attr("type","password");
        $('#'+img).attr('src',"i/show@2x.png");
    }
}
