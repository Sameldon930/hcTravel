...
@section('content')
    <div class="box">
        <div class="padding-top-sm">
            <a href="{{route('admin.goods.add')}}">
                <button class="pretty-btn">二手商品添加</button>
            </a>
        </div>
    </div>
                ...
                <tr>
                    ...
                    <td>
                        <div class="switch " data-target="operate-status"
                             data-url="{{route('admin.goods.switch',
                                         ['goods'=>$data->id])}}" data-id="{{$data->id}}"
                        >
                            <input type="checkbox" name="status" value="{{ $data->status }}"
                                   @if( $data->status == 1 ) checked @endif/>
                        </div>
                    </td>
                    <td><a href="{{route('admin.goods.edit',['finance'=>$data->id])}}" class="pretty-btn">修改</a></td>
                </tr>
            @endforeach
